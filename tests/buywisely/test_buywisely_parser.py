def test_hydration_parser_with_g85_product():
    ...
    with open(
        "tests/buywisely/fixtures/real_buywisely_motorola-moto-g85-5g-128gb-urban-grey-.html",
        "r",
        encoding="utf-8",
    ) as f:
        html_content = f.read()
    ...
    assert (
        "Motorola" in product_data.get("name", "")
        or "motorola" in product_data.get("name", "").lower()
    )
...
def test_hydration_parser_with_real_data():
    ...
    print(
        f"[DIAG] name: {name}, title: {title}, name_nested: {name_nested}, title_nested: {title_nested}"
    )

    expected_name = "Motorola Moto G75 5G 256GB Grey with Buds"
    if (
        name == expected_name
        or title == expected_name
        or name_nested == expected_name
        or title_nested == expected_name
    ):
        pass
    else:
        raise AssertionError(
            f"Expected product name '{expected_name}' not found in any key. product_data: {product_data}"
        )
    ...
    found = any(
        (offer.get("seller") == "VTech Industries")
        or (
            isinstance(offer.get("seller"), dict)
            and offer["seller"].get("name") == "VTech Industries"
        )
        for offer in offers
