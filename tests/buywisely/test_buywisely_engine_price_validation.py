...
async def test_price_must_be_greater_than_zero(mock_fetch_seller_price, mock_safe_request):
    ...
    assert extracted_price is not None, "Extracted price should not be None for a valid product"
    ...
    @patch("custom_components.price_tracker.services.buywisely.engine.SafeRequest")
    @patch("custom_components.price_tracker.services.buywisely.data_transformer._fetch_and_parse_seller_price")
    async def test_price_extraction_with_real_seller_page(mock_fetch_seller_price, mock_safe_request):
        """Test price extraction using a real seller page HTML fixture."""
        sample_html_real = _read_fixture_html("real_buywisely_motorola-moto-g75-5g-256gb-grey-with-buds.html")
        ...
        mock_fetch_seller_price.return_value = 399.00  # Simulate expected price from seller page
        engine_real = BuyWiselyEngine(item_url="https://www.buywisely.com.au/product/real-fixture-test", request_cls=mock_safe_request)
        result_real = await engine_real.load()
        print("[DIAG] Extraction result from real seller page:", result_real)
        assert result_real is not None, "Expected result, got None from real seller page"
        extracted_price_obj = getattr(result_real, 'price', None)
        assert extracted_price_obj is not None, "Expected a price object from real seller page"
        extracted_price = getattr(extracted_price_obj, 'price', None)
        assert extracted_price is not None, "Extracted price should not be None for real seller page"
        assert extracted_price > 0.0, "Price should be greater than 0 for real seller page"
