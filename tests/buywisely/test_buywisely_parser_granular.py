...
def test_hydration_parser_offer_fields():
    ...
    if isinstance(parsed, list):
        for idx, obj in enumerate(parsed):
            if isinstance(obj, dict) and 'offers' in obj:
                print(f"[DIAG] Product dict {idx} with offers: keys={list(obj.keys())}, product={obj}")
    ...
    assert 'price' in offer or 'base_price' in offer, f"Offer missing price and base_price. Offer: {offer}"
    assert 'seller_product_url' in offer, f'Offer missing seller_product_url. Offer: {offer}'
