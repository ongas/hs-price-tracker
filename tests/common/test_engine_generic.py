...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            pytest.importorskip(
                "custom_components.price_tracker.services.coupang.engine"
            ).CoupangEngine,
        ...
        pytest.param(
            pytest.importorskip(
                "custom_components.price_tracker.services.kurly.engine"
            ).KurlyEngine,
        ...
        pytest.param(
            pytest.importorskip(
                "custom_components.price_tracker.services.rankingdak.engine"
            ).RankingdakEngine,
        ...
        pytest.param(
            pytest.importorskip(
                "custom_components.price_tracker.services.smartstore.engine"
            ).SmartstoreEngine,
        ...
        pytest.param(
            pytest.importorskip(
                "custom_components.price_tracker.services.ssg.engine"
            ).SsgEngine,
...
async def test_engine_load(engine_cls, item_url, mock_response):
    if mock_response:
        with patch(
            "custom_components.price_tracker.services.coupang.engine.SafeRequest.request",
            new_callable=AsyncMock,
        ) as mock_request:
            mock_request.return_value = MagicMock(
                data=json.dumps(mock_response), is_not_found=False, has=True
            )
