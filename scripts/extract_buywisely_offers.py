...
async def extract_offers(source: str, output_file: str = None):
    ...
    if output_file:
        ...
        with open(output_path, 'w', encoding='utf-8') as f:
            ...
            f.write("Index\tSeller_Name\tSeller_ID\tBase_Price\tShipping\tCALC_Total_Price\t")
            ...
            f.write("Cashrewards_ID\tCashrewards_Name\tCashrewards_Commission_String\tCashrewards_Commission\t")
            f.write("Cashrewards_Hyphenated_Name\tCashrewards_Logo\tCashrewards_Online\tCashrewards_Instore\t")
            f.write("Cashrewards_Hostname\tCashrewards_Created_At\tCashrewards_Updated_At\n")
            ...
            for idx, offer in enumerate(offers, 1):
                ...
                shopback_id = shopback_obj.get('sb_id', 'N/A') if shopback_obj else 'N/A'
                shopback_name = shopback_obj.get('sb_name', 'N/A') if shopback_obj else 'N/A'
                shopback_url = shopback_obj.get('url', 'N/A') if shopback_obj else 'N/A'
                shopback_commission = shopback_obj.get('commission', 'N/A') if shopback_obj else 'N/A'
                shopback_created_at = shopback_obj.get('created_at', 'N/A') if shopback_obj else 'N/A'
                shopback_updated_at = shopback_obj.get('updated_at', 'N/A') if shopback_obj else 'N/A'
                shopback_hostname = shopback_obj.get('hostname', 'N/A') if shopback_obj else 'N/A'

                # Extract ALL cashrewards details
                cashrewards_id = cashrewards_obj.get('cr_id', 'N/A') if cashrewards_obj else 'N/A'
                cashrewards_name = cashrewards_obj.get('name', 'N/A') if cashrewards_obj else 'N/A'
                cashrewards_commission_string = cashrewards_obj.get('commission_string', 'N/A') if cashrewards_obj else 'N/A'
                cashrewards_commission = cashrewards_obj.get('commission', 'N/A') if cashrewards_obj else 'N/A'
                cashrewards_hyphenated_name = cashrewards_obj.get('hyphenated_name', 'N/A') if cashrewards_obj else 'N/A'
                cashrewards_logo = cashrewards_obj.get('logo', 'N/A') if cashrewards_obj else 'N/A'
                cashrewards_online = cashrewards_obj.get('online', 'N/A') if cashrewards_obj else 'N/A'
                cashrewards_instore = cashrewards_obj.get('instore', 'N/A') if cashrewards_obj else 'N/A'
                cashrewards_hostname = cashrewards_obj.get('hostname', 'N/A') if cashrewards_obj else 'N/A'
                cashrewards_created_at = cashrewards_obj.get('created_at', 'N/A') if cashrewards_obj else 'N/A'
                cashrewards_updated_at = cashrewards_obj.get('updated_at', 'N/A') if cashrewards_obj else 'N/A'
                ...
                # Write row
                f.write(f"{idx}\t{seller_name}\t{seller_id}\t{base_price}\t{shipping}\t{total_price:.2f}\t")
                f.write(f"{created_at}\t{condition}\t{has_shopback}\t{has_cashrewards}\t")
                f.write(f"{seller_url}\t{product_url}\t{offer_id}\t{pid}\t{country_code}\t")
                # Shopback data (all fields)
                f.write(f"{shopback_id}\t{shopback_name}\t{shopback_url}\t{shopback_commission}\t")
                f.write(f"{shopback_created_at}\t{shopback_updated_at}\t{shopback_hostname}\t")
                # Cashrewards data (all fields)
                f.write(f"{cashrewards_id}\t{cashrewards_name}\t{cashrewards_commission_string}\t{cashrewards_commission}\t")
                f.write(f"{cashrewards_hyphenated_name}\t{cashrewards_logo}\t{cashrewards_online}\t{cashrewards_instore}\t")
                f.write(f"{cashrewards_hostname}\t{cashrewards_created_at}\t{cashrewards_updated_at}\n")
