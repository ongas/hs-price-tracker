...
def normalize_js_dates(js_str: str) -> str:
    ...
    return re.sub(r'"\$D([0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}\.[0-9]{3}Z)', r'"\1"', js_str)
...
def parse_js_array_literal(js_str):
    ...
    try:
        ...
        _LOGGER.warning(f"Failed to parse JS array literal with json.loads: {e}. Attempting with demjson3.")
        try:
