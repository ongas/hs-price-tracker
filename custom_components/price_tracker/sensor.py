...
async def async_setup_entry(
    ...
    _LOGGER.info(
        "[DIAG][sensor.py] async_setup_entry called for entry_id=%s, config=%s",
    ...
    try:
        _LOGGER.info(
            "[DIAG][sensor.py] Registering sensor for item_url: %s, proxy: %s, proxy_opensource: %s",
            item_url,
            proxy,
            proxy_opensource,
        )
        ...
        engine = create_service_engine(hass, service_type)(
            item_url=item_url,
            proxies=proxy,
            device=device,
            selenium=selenium,
        ...
        _LOGGER.info(
            ...
            engine,
            getattr(engine, "id_str", lambda: None)(),
            getattr(engine, "entity_id", None),
        )
        sensor = PriceTrackerSensor(
            engine=engine,
            device=device,
            unit_type=ItemUnitType.of(
                Lu.get(config, CONF_ITEM_UNIT_TYPE, "auto")
            )  # Use config directly
            ...
            refresh_period=Lu.get(
                config, CONF_ITEM_REFRESH_INTERVAL, 30
            ),  # Use config directly
            management_category=Lu.get(
                config, CONF_ITEM_MANAGEMENT_CATEGORY, None
            ),  # Use config directly
        ...
        _LOGGER.info(
            "[DIAG][sensor.py] Created sensor: %s, sensor.entity_id: %s, sensor._attr_unique_id: %s",
            sensor,
            getattr(sensor, "entity_id", None),
            getattr(sensor, "_attr_unique_id", None),
        )
    ...
    _LOGGER.info(
        "[DIAG][sensor.py] Calling async_add_entities with sensors: %s", sensors
    )
