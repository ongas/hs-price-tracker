        ...
        try:
            if user_input is None or "product_url" not in user_input:
                ...
                return self._config_flow.async_show_form(
                    ...
                    errors={},
                    description_placeholders={"description": "Enter the product URL to track with Buywisely.", "title": "Price Tracker Setup"},
                )
            if "service_type" not in user_input:
                _LOGGER.error("[PriceTrackerSetup] user_input missing 'service_type'. Cannot proceed with setup. user_input=%s", user_input)
                return self._config_flow.async_show_form(
                    step_id="user",
                    data_schema=None,
                    errors={"base": "missing_service_type"},
                    description_placeholders={"description": "Service type is required. Please select a valid service.", "title": "Price Tracker Setup"},
                )
            ...
            _LOGGER.error("[PriceTrackerSetup] Exception in setup: %s", e, exc_info=True)
            return self._config_flow.async_show_form(
                step_id="user",
                data_schema=None,
                errors={"base": "exception", "details": str(e)},
                description_placeholders={"description": f"Error: {str(e)}", "title": "Price Tracker Setup"},
            )
