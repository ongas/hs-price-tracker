...
def step_when_product_loaded(context):
    context.logs.append(
        "Product loaded: https://www.buywisely.com.au/product/test-product?id=12345"
    )
    context.logs.append(
        "Extracted data: {id: 12345, name: 'Test Product', price: 95.0}"
    )
    context.logs.append("Error: None")


@then(
    "diagnostic logs are generated including product URLs, IDs, extracted data, and error messages"
)
