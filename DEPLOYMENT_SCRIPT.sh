...
SRC_DIR="$(dirname "$0")"
COMPONENT_DIR="$SRC_DIR/custom_components/price_tracker"
...
rsync -av --exclude='tests' --exclude='backup*' --exclude='*.md' --exclude='*.pyc' --exclude='__pycache__' --exclude='.ruff_cache' --exclude='.pytest_cache' --exclude='.git' --exclude='.github' --exclude='.DS_Store' --exclude='.docs' --exclude='images' "$COMPONENT_DIR/" "$TARGET_DIR/"
