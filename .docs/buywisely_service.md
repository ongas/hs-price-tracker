...
6.  **Verification (HA API):**
    ...
    *   Use the Python script `get_ha_token_and_curl.py` to securely access the API. This script will automatically retrieve the API token from your `secrets.yaml` and make the `curl` request.
    *   Navigate to the `custom_components/price_tracker/scripts/` directory.
    *   Execute the script: `python get_ha_token_and_curl.py`
